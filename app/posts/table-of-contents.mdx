...
Imagine you are building a blog and each of your posts have a table of contents to indicate the different
logic sections of the blog. I am using a combination of Remix and MDX to render each post and keeping track of
the sections in the frontmatter - if these words aren't making a lot of sense to you, don't fret!
There are tons of resources of how to setup a blog using MDX and your framework of choice (like Next.js, Remix, Gatsby, etc)
and you can quickly get upto speed! If these terms already make sense to you, read on.

I built out a very primitive component to showcase the sections of the post, and it looks like this:

<img
  src="https://thisisktm.com/posts/resources/toc/toc-1"
...
in the table of contents, and be able to identify which section they are currently reading with a visual cue. The former seems to work as expected, but we need to
...
active section in the table of contents. The premise seems simple, but I quickly found out it was not as straightforward as it seems!

Let's start with the basics: Each header (represented by a `h2`) in the article has an `id` associated with it, which is just a camel-cased version of a header.
(For example an `h2` header with the text "Approaching the problem" will have an associated `id`: `approaching-the-problem`)

Once the component mounts, we need to keep track, observe each of these section headers and capture when they intesect with the viewport. When they do, we can
...
<img
  src="https://thisisktm./posts/resources/toc/toc-2"
...
This property captures information about the `size` and `position` relative to the viewport, and we are interested in the `y` property on this object, that tells us
...
<img
  src="https://thisisktm.com/posts/resources/toc/toc-3"
...
in components and viewports, it's a simple, yet powerful tool that should definitely be part of your developer toolkit!
